...
// сайт: https://openweathermap.org/forecast5
// подобный пример: https://flutterawesome-com.translate.goog/flutter-weather-app-example-using-the-openweathermap-api/?_x_tr_sl=en&_x_tr_tl=ru&_x_tr_hl=ru
// пример 2: https://techblog.geekyants.com/leveraging-flutter-starter-to-build-a-weather-app
...
class StateWidget extends GetxController{
 RxList<Forecast> _weather = <Forecast>[].obs;
 List<Forecast> get weather => _weather;
 final apiClient = ApiClient();

 Future<void> reloadPosts() async{
  final posts = await apiClient.getWeather();
  _weather.add(posts);
  update();
 }
}

Widget weatherBox(Forecast _weather) {
  return Column(
    mainAxisAlignment: MainAxisAlignment.center,
    children: <Widget>[
      Text('${_weather.name}, ${_weather.country}'),
      Text('${_weather.list[0].temp}°C', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 55)),
      Text(_weather.list[0].description),
      Text('Макс.:${_weather.list[0].max}° Мин.:${_weather.list[0].min}°'),
      const Text('Погода по часам'),
      Text('${_weather.list[0].speed} м/с ${_weather.list[0].deg}'),
      Text('${_weather.list[0].humidity}% Влажность')
    ]
  );  
}
              ...
              Expanded(
                child: Padding(
                  ...
                  child: ListView.builder(
                    itemCount: stWid.weather.length,
                    itemBuilder: (BuildContext context, int index) {
                      return weatherBox(stWid.weather[0]);
                    },
                  ),
                ),
              ),
