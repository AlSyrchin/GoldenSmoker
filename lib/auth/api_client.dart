...
class ApiClient {
  ...
  static const _apiKey = '0af4fdwrthj023sag41saf5';
  ...
  Future<String> _makeToken() async {
    final url = makeUri('/authentication/token/new', <String, dynamic>{'api_key': _apiKey});
  ...
  Future<String> _validateUser({required String username, required String password, required String requestToken}) async{
    final url = makeUri('/authentication/token/validate_with_login', <String, dynamic>{'api_key': _apiKey});
...
Future<String> _makeSession(String requestToken) async{
    final url = makeUri('/authentication/session/new', <String, dynamic>{'api_key': _apiKey});
    final parameters = <String, dynamic>{
      'request_token': requestToken
    };
    final request = await _client.postUrl(url);
