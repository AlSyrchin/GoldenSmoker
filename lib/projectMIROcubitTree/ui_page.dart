...
class WeatherSearchPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final recCubit = context.read<RecipeCubit>(); 
    return Scaffold(
      appBar: AppBar(
...
BlocBuilder<RecipeCubit, ReciepState>(
          builder: (context, state) {
            if (state is RecInitial) {
              return buildInitialInput();
            } else if (state is RecLoading) {
              return buildLoading();
            } else if (state is RecLoaded) {
              return StepsReorder(state.list);
            } else {
      ...
      SizedBox(
          ...
          child: ReorderableListView.builder(
              ...
              itemBuilder: (context, index) => ListTile(
                ...
                title: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      // Expanded(
                      //     child: Text(
                      //         '${DateFormat.m().format(DateTime.parse('00000000T${listStage[index].time}'))} мин',
                      //         style: t32w700)),
                      Expanded(
                          child: Text(
                ...
                trailing: IconButton(
                    onPressed: () => {recCubit.removeEtap(index)},
                    // removeIndex(index),
              ...
              onReorder: (oldIndex, newIndex) => {},
                  // fromTo(oldIndex, newIndex),
              footer: InkWell(
                onTap: () => {recCubit.addEtap()}, //Navigator.push(context, MaterialPageRoute(builder: (context) => NewStep()))
                child: Container(
